...
pub async fn create_session(configuration: &configuration::Configuration, idempotency_key: Option<&str>, session_request: Option<models::SessionRequest>) -> Result<models::SessionResponse, Error<CreateSessionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_idempotency_key = idempotency_key;
    let p_body_session_request = session_request;

    let uri_str = format!("{}/v2/sessions", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_session_request);
...
pub async fn get_session(configuration: &configuration::Configuration, session_id: &str, idempotency_key: Option<&str>) -> Result<models::SessionResponse, Error<GetSessionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_session_id = session_id;
    let p_header_idempotency_key = idempotency_key;

    let uri_str = format!("{}/v2/sessions/{sessionId}", configuration.base_path, sessionId=crate::apis::urlencode(p_path_session_id));
