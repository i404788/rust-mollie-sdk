...
pub async fn create_client_link(configuration: &configuration::Configuration, idempotency_key: Option<&str>, client_link_request: Option<models::ClientLinkRequest>) -> Result<models::ClientLinkResponse, Error<CreateClientLinkError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_idempotency_key = idempotency_key;
    let p_body_client_link_request = client_link_request;

    let uri_str = format!("{}/v2/client-links", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_client_link_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ClientLinkResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ClientLinkResponse`")))),
        }
    } else {
        let content = resp.text().await?;
