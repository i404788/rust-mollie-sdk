...
pub async fn create_transfer(configuration: &configuration::Configuration, x_client_signature: &str, x_client_signed_at: &str, idempotency_key: &str, idempotency_key2: Option<&str>, transfer_request: Option<models::TransferRequest>) -> Result<models::TransferResponse, Error<CreateTransferError>> {
    ...
    let p_header_idempotency_key = idempotency_key2;
    ...
    req_builder = req_builder.header("X-Client-Signature", p_header_x_client_signature.to_string());
    req_builder = req_builder.header("X-Client-Signed-At", p_header_x_client_signed_at.to_string());
...
pub async fn get_transfer(configuration: &configuration::Configuration, business_accounts_transfer_id: &str, testmode: Option<bool>, idempotency_key: Option<&str>) -> Result<models::TransferResponse, Error<GetTransferError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_business_accounts_transfer_id = business_accounts_transfer_id;
    let p_query_testmode = testmode;
    let p_header_idempotency_key = idempotency_key;

    let uri_str = format!("{}/v2/business-accounts/transfers/{businessAccountsTransferId}", configuration.base_path, businessAccountsTransferId=crate::apis::urlencode(p_path_business_accounts_transfer_id));
