...
**lines** | Option<[**Vec<models::SessionLineItem>**](SessionLineItem.md)> | List of items the customer will pay for in this session. The sum of all line items must equal the session's amount.  All lines must have the same currency as the session. | [optional]
